...
# Note: We manually removed the dependency on convlab for this version of the model
#
...
class ContextBERT_ERToD(nn.Module):
    ...
    def __init__(
        ...
        # Note: We removed the dependency on convlab for this version of the model.
        #
        # Original code:
        # > self.vectoriser = VectorBinary()
        #
        # Patched code:
        class VectorBinaryPatch:
            def __init__(self):
                self.ds_dim = ds_dim
            ...
            def state_vectorize(
                ...
                return np.zeros((self.ds_dim,))

        self.vectoriser = VectorBinaryPatch()
    ...
    def normalise_input(self, user_utt: str = "", dialog_state_history: list | None = None) -> tuple:
        ...
        # Note: might need to use [0] for vectorised state
