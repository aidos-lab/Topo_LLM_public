...
class SubsetSamplerRandom:
    ...
    def sample_subsets(
        ...
        if len(array) >= self.embeddings_data_prep_sampling_config.num_samples:
            subsample_idx_vector: np.ndarray = self.rng.choice(
                range(len(array)),
                replace=False,
                size=self.embeddings_data_prep_sampling_config.num_samples,
            )
        else:
            subsample_idx_vector: np.ndarray = self.rng.choice(
                range(len(array)),
        ...
        if self.verbosity >= Verbosity.NORMAL:
            ...
            self.logger.info(
                f"Expected sample size: {self.embeddings_data_prep_sampling_config.num_samples = }",  # noqa: G004 - low overhead
            )
