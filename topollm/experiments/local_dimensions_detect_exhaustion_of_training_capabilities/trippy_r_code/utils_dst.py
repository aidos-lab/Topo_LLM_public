...
class TrippyDataset(Dataset):
    def __init__(
        self, args, examples, model, tokenizer, processor, dset="train", evaluate=False, automatic_labels=None
    ):
        self.args = args
    ...
    def load_features_from_file(self, cached_file):
        ...
        self.features, _ = torch.load(cached_file)
    ...
    def save_features_to_file(self, cached_file):
        ...
        torch.save((self.features, None), cached_file)
            ...
            for value in self.value_list[slot]:
                ...
                if value in self.label_maps:
                    for variant in self.label_maps[value]:
                        ...
                        encoded_slot_value, _ = self._encode_text(
                            text, input_ids.unsqueeze(0), input_mask.unsqueeze(0), mode=val_rep_mode
                        )
                ...
                if (
                    self.dropout_value_list is not None
                    and slot in self.dropout_value_list
                    and value in self.dropout_value_list[slot]
                ):
                    for dropout_value_seq in self.dropout_value_list[slot][value]:
                        ...
                        encoded_slot_value_dropped_out, _ = self._encode_text(
                            text, input_ids.unsqueeze(0), input_mask.unsqueeze(0), mode=val_rep_mode
                        )
                        if isinstance(encoded_slot_value_dropped_out, dict):
                            encoded_slot_value_dropped_out = encoded_slot_value_dropped_out[
                                slot
                            ]  # Keep only slot-specific encoding
                        encoded_slot_value_dropped_out = encoded_slot_value_dropped_out.cpu()
                        self.encoded_dropout_slot_values[slot][tuple(dropout_value_seq)] = (
                            encoded_slot_value_dropped_out
                        )
    ...
    def encode_slots(self, train=False):
        ...
        for slot in self.slot_dict:
            ...
            input_ids, input_mask = self._build_input(text)
            encoded_slot_pooled, encoded_slot_seq = self._encode_text(
                text, input_ids.unsqueeze(0), input_mask.unsqueeze(0), mode="encode", train=train
            )
    ...
    def tokenize_sequences(self, max_len=1, train=False):
        ...
        for f in tqdm(self.features, desc="Tokenize sequences"):
            seq = f.input_ids[
                1 : f.input_ids.index(self.tokenizer.pad_token_id) if self.tokenizer.pad_token_id in f.input_ids else -1
            ]
            example_id = f.guid
            ...
            for t_itr, t in enumerate(token_seq):
                if (
                    ("roberta" in self.args.model_type and t[0] == "\u0120")
                    or t[0:2] != "##"
                    or t
                    in [
                    ...
                    or t in self.tokenizer.additional_special_tokens
                ):
                    word_list.append([t])
    ...
    def update_samples_for_proto(self, max_len=1):
        ...
        for index in tqdm(range(self.size), desc="Update negative samples for proto training"):
            ...
            if b_index == 0:
                ...
                result["input_ids"] = torch.tensor([self.features[i].input_ids for i in range(index, index + offset)])
                result["example_id"] = torch.tensor([i for i in range(index, index + offset)])
        ...
        for i, input_ids_dropout in tqdm(enumerate(self.data["input_ids_dropout"]), desc="Dropout inputs"):
            ...
            if self.args.svd > 0.0:
                ...
                if self.args.use_td:
                    ...
                    while (
                        self.tokenizer.sep_token_id in random_token_id
                        or self.tokenizer.pad_token_id in random_token_id
                        or self.tokenizer.cls_token_id in random_token_id
                    ):
                        random_token_id = random.sample(range(top_n), len(indices_to_drop_out_noncat))
            ...
            for slot in self.slot_list:
                ...
                if len(spans) > 0 and not torch.equal(
                    self.data["input_ids"][i][spans[0]], self.data["input_ids_dropout"][i][spans[0]]
                ):
    ...
    def __getitem__(self, index):
        ...
        result["start_pos"] = {}
        for e in getattr(self.features[index], "start_pos"):
            result["start_pos"][e] = torch.zeros(len(self.features[index].input_ids), dtype=torch.long)
            ones = torch.tensor(getattr(self.features[index], "start_pos")[e])
            if len(ones) > 0:
                result["start_pos"][e][ones] = 1
        for key in ["refer_id", "diag_state", "class_label_id"]:
            result[key] = {}
            for e in getattr(self.features[index], key):
                result[key][e] = torch.tensor(getattr(self.features[index], key)[e])
        result["inform_slot_id"] = {}
        for e in getattr(self.features[index], "inform_slot"):
            result["inform_slot_id"][e] = torch.tensor(getattr(self.features[index], "inform_slot")[e])
        result["example_id"] = torch.tensor(index)
        ...
        if self.mode == "proto":
            ...
            for slot in self.slot_list:
                ...
                if self.encoded_slot_values is not None:
                    ...
                    result["dropout_value_feat"][slot] = torch.zeros(
                        (1, self.model.config.hidden_size), dtype=torch.float
                    )
                    ...
                    if token_is_pointable and pos_value in self.encoded_slot_values[slot]:
                        ...
                        if self.dropout_value_seq is not None:
                            if result["example_id"].item() in self.dropout_value_seq[slot]:
                                ...
                                result["dropout_value_feat"][slot] = self.encoded_dropout_slot_values[slot][
                                    dropout_value_seq
                                ]
    ...
    def _convert_examples_to_features(
        self, examples, slot_list, class_list, model_type, max_seq_length, automatic_labels=None
    ):
        """Loads a data file into a list of `InputBatch`s."""

        if model_type == "roberta":
            model_specs = {"MODEL_TYPE": "roberta", "TOKEN_CORRECTION": 6}
        else:
            model_specs = {"MODEL_TYPE": "bert", "TOKEN_CORRECTION": 4}
        ...
        def _truncate_length_and_warn(tokens_a, tokens_b, history, max_seq_length, model_specs, guid):
            ...
            max_len = (
                max_seq_length
                - model_specs["TOKEN_CORRECTION"]
                - len(history) * 2
                - self.args.tag_none_target * int(model_specs["TOKEN_CORRECTION"] / 2)
            )
        ...
        def _get_transformer_input(tokens_a, tokens_b, history, max_seq_length, model_specs):
            ...
            if model_specs["MODEL_TYPE"] == "roberta":
                if self.args.tag_none_target:
                    ...
                    usr_mask = (
                        [0]
                        + [1, 0, 0]
                        + len(tokens_a) * [0 if self.args.swap_utterances else 1]
                        + 2 * [0]
                        + len(tokens_b) * [1 if self.args.swap_utterances else 0]
                        + 2 * [0]
                    )
                else:
            ...
            for hst_itr in range(len(history)):
                ...
                usr_mask += (
                    len(hst_a) * [0 if self.args.swap_utterances else 1]
                    + [0]
                    + len(hst_b) * [1 if self.args.swap_utterances else 0]
                    + [0]
                )
            tokens.append(sep)
            if model_specs["MODEL_TYPE"] == "roberta":
        ...
        for example_index, example in enumerate(examples):
            ...
            tokens_a, token_to_subtoken_a = _tokenize_text(example.text_a, example.text_a_label, model_specs)
            tokens_b, token_to_subtoken_b = _tokenize_text(example.text_b, example.text_b_label, model_specs)
            ...
            tokens_dict[(re_guid_diag, re_guid_turn)] = [
                [tokens_a, token_labels_a_dict],
                [tokens_b, token_labels_b_dict],
            ]
        ...
        for example_index, example in enumerate(examples):
            ...
            tokens, input_ids, input_mask, segment_ids, usr_mask, hst_boundaries = _get_transformer_input(
                tokens_a, tokens_b, tokens_history, max_seq_length, model_specs
            )
            ...
            token_label_ids = _get_token_label_ids(
                token_labels_a_dict, token_labels_b_dict, token_labels_history_dict, max_seq_length, model_specs
            )
            ...
            for slot in slot_list:
                ...
                start_pos_dict[slot] = torch.where(torch.tensor(token_label_ids[slot]) == 1)[0].tolist()
