...
def get_turn_label(turn, prev_dialogue_state, slot_list, dial_id, turn_id, sys_inform_dict,
    ...
    for slot_type in slot_list:
        updated_slots_dict[slot_type] = 0
        if slot_type in cur_ds_dict and slot_type in prev_ds_dict and cur_ds_dict[slot_type] != prev_ds_dict[slot_type]:
            updated_slots_dict[slot_type] = 1
        ...
        if (dial_id, turn_id, slot_type) in sys_inform_dict:
            inform_label_dict[slot_type] = sys_inform_dict[(dial_id, turn_id, slot_type)]
            inform_slot_label_dict[slot_type] = 1
        if class_type == 'inform' and inform_label_dict[slot_type] != cur_ds_dict[slot_type]:
            class_type = 'none'
    ...
    for entry in input_data:
        ...
        for turn_id, turn in enumerate(entry['turns']):
            ...
            (text_a,
             ...
             cur_ds_dict,
             class_label,
             updated_slots) = get_turn_label(turn,
                                             prev_ds,
                                             slot_list,
                                             dial_id,
                                             turn_id,
...
def get_value_list(input_file, slot_list):
    ...
    for entry in input_data:
        for turn in entry['turns']:
            cur_ds_dict = dialogue_state_to_sv_dict(turn['dialogue_state'])
            for slot in slot_list:
                if slot in cur_ds_dict and cur_ds_dict[slot] not in exclude:
                    if cur_ds_dict[slot] not in value_list[slot]:
                        value_list[slot][cur_ds_dict[slot]] = 0
                    value_list[slot][cur_ds_dict[slot]] += 1
    return value_list
