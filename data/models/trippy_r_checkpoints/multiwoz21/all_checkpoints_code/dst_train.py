...
def train(args, train_dataset, dev_dataset, automatic_labels, model, tokenizer, processor):
    ...
    for e_itr, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            epoch_iterator.set_postfix({'loss': loss.item(), 'cl': cl_loss.item(), 'tk': tk_loss.item(), 'tp': tp_loss.item()})
        ...
        if args.local_rank == -1 and dev_dataset is not None:
            results = evaluate(args, dev_dataset, model_single_gpu, tokenizer, processor, no_print=True, no_output=True, prefix=global_step)
...
def evaluate(args, dataset, model, tokenizer, processor, no_print=False, no_output=False, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if not no_print:
            predict_and_print(args, model, tokenizer, batch, outputs, unique_ids, input_ids, values, inform, ds, value_match)
        if not no_output:
    ...
    output_prediction_file = os.path.join(args.output_dir, "pred_res%s.%s.%s.json" % (args.cache_suffix, args.predict_type, prefix))
    if not no_output:
...
def eval_metric(args, model, tokenizer, batch, outputs, threshold=0.0, dae=False, values=None, value_match=None):
    ...
    for slot in model.slot_list:
        ...
        token_is_referrable = torch.eq(class_label_id, class_types.index('refer') if 'refer' in class_types else -1).float()
...
def smooth_roberta_predictions(pred, input_tokens, tokenizer):
    ...
    while i < len(pred):
        ...
        elif span and input_tokens[i][0] != "\u0120" and input_tokens[i] not in [tokenizer.unk_token, tokenizer.bos_token,
        ...
        elif span and (input_tokens[i][0] == "\u0120" or input_tokens[i] in [tokenizer.unk_token, tokenizer.bos_token,
                                                                             tokenizer.eos_token, tokenizer.unk_token,
                                                                             tokenizer.sep_token, tokenizer.pad_token,
                                                                             tokenizer.cls_token, tokenizer.mask_token]):
            span = False
    ...
    while i >= 0:
        if pred[i] > 0:
            span = True
        if span and input_tokens[i][0] != "\u0120" and input_tokens[i] not in [tokenizer.unk_token, tokenizer.bos_token,
                                                                               tokenizer.eos_token, tokenizer.unk_token,
                                                                               tokenizer.sep_token, tokenizer.pad_token,
                                                                               tokenizer.cls_token, tokenizer.mask_token]:
            smoothed_pred[i] = 1 # use label for in-span tokens
...
def predict_and_format(args, model, tokenizer, processor, batch, outputs, ids, input_ids_unmasked, values, inform, ds, value_match, dae=False):
    def _tokenize(text):
        if "\u0120" in text:
            text = re.sub(" ", "", text)
            text = re.sub("\u0120", " ", text)
    ...
    for i in range(len(ids)):
        ...
        for slot in model.slot_list:
            ...
            prediction['start_prediction_%s' % slot] = [i for i in range(len(start_prediction)) if start_prediction[i] > 0]
            prediction['start_confidence_%s' % slot] = [norm_logits[j].item() for j in range(len(start_prediction)) if start_prediction[j] > 0]
            ...
            if value_logits is not None:
                prediction['value_prediction_%s' % slot] = [i for i in range(len(value_prediction)) if value_prediction[i] > 0]
                prediction['value_confidence_%s' % slot] = [value_logits[i] for i in range(len(value_logits)) if value_prediction[i] > 0]
            ...
            if len(prediction['value_prediction_%s' % slot]) > 0:
                ...
                prediction_addendum['slot_value_confidence_%s' % slot] = np.max(prediction['value_confidence_%s' % slot])
    ...
    for i in range(len(ids)):
        ...
        for s in model.slot_list:
            ...
            if per_slot_att_weights[s] is not None:
                ...
                class_norm_weights[s] = torch.clamp(class_weights - torch.mean(class_weights), min=0) / torch.max(class_weights)
            token_weights = per_slot_start_logits[s][i][:len(input_ids)]
            token_norm_weights[s] = torch.clamp(token_weights - torch.mean(token_weights), min=0) / torch.max(token_weights)
